...
AND characters.charId=death.chrId;";
        ...
        public QuestItem GetDailyQuest(string accId, XmlData data)
        {
            ...
            using (var rdr = cmd.ExecuteReader())
            {
                while (rdr.Read())
                {
                    ...
                    if (goals.Length < DailyQuestConstants.QuestsPerDay)
                        break;
                    quest = new QuestItem
                    {
        ...
        public QuestItem GenerateDailyQuest(string accId, XmlData data, DateTime pst5pm)
        {
            ...
            do
            {
                ...
                while (items.Contains(item))
                    item = candidates[(r = rand.Next(candidates.Count))].ObjectType;
                items.Add(item);
        ...
        public Account GetAccount(string id, XmlData data, string uuid = null, string password = null)
        {
            if (String.IsNullOrWhiteSpace(id))
                return CreateGuestAccount(id ?? String.Empty);
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText =
            ...
            using (MySqlDataReader rdr = cmd.ExecuteReader())
            {
                ...
                ret = new Account
                {
                    ...
                    MarksEnabled = rdr.GetInt32("marksEnabled"),
                    CookingLevel = rdr.GetInt32("cookingLevel"),
                };
            }
            ReadStats(ret);
        ...
        public int UpdateOnraneCurrency(Account acc, int amount)
        {
            ...
            cmd.CommandText = @"UPDATE stats SET onrane = onrane + (@amount) WHERE accId =@accId;
SELECT onrane FROM stats WHERE accId=@accId;";
            cmd.Parameters.AddWithValue("@accId", acc.AccountId);
            cmd.Parameters.AddWithValue("@amount", amount);
            return (int)cmd.ExecuteScalar();
        }

        public int UpdateKantosCurrency(Account acc, int amount)
        {
            MySqlCommand cmd = CreateQuery();
            if (amount > 0)
            {
                cmd.CommandText = "UPDATE stats SET totalKantos = totalKantos + @amount WHERE accID=accId;";
                cmd.Parameters.AddWithValue("@accId", acc.AccountId);
                cmd.Parameters.AddWithValue("@amount", amount);
                cmd.ExecuteNonQuery();
                cmd = CreateQuery();
            }
            cmd.CommandText = @"UPDATE stats SET kantos = kantos + (@amount) WHERE accId =@accId;
SELECT kantos FROM stats WHERE accId=@accId;";
            cmd.Parameters.AddWithValue("@accId", acc.AccountId);
            cmd.Parameters.AddWithValue("@amount", amount);
            return (int)cmd.ExecuteScalar();
        }
...
SELECT credits FROM stats WHERE accId=@accId;";
        ...
        public int UpdateCookingLevel(Account acc, int amount)
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText = @"UPDATE accounts SET cookingLevel = cookingLevel + (@amount) WHERE id=@id;
SELECT accounts FROM cookingLevel WHERE id=@id;";
            cmd.Parameters.AddWithValue("@id", acc.AccountId);
            cmd.Parameters.AddWithValue("@amount", amount);
            return (int)cmd.ExecuteScalar();
        }

        public void UpdateXpLevel(Account acc, decimal amount)
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText = @"UPDATE accounts SET cookingXp = cookingXp + (@amount) WHERE id=@id;
SELECT cookingXp FROM accounts WHERE id=@id;";
            cmd.Parameters.AddWithValue("@id", acc.AccountId);
            cmd.Parameters.AddWithValue("@amount", amount);
            cmd.ExecuteNonQuery();
        }

        public void resetXp(Account acc)
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText = @"UPDATE accounts SET cookingXp = 0.00 where id=@id";
            cmd.Parameters.AddWithValue("@id", acc.AccountId);
            cmd.ExecuteNonQuery();
        }

        public decimal GetCookingXpLevel(Account acc)
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText = @"SELECT cookingXp FROM accounts WHERE id=@id";
            cmd.Parameters.AddWithValue("@id", acc.AccountId);
            return (decimal)cmd.ExecuteScalar();
        }

        public int GetCookingLevel(Account acc)
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText = @"SELECT cookingLevel FROM accounts WHERE id=@id";
            cmd.Parameters.AddWithValue("@id", acc.AccountId);
            return (int)cmd.ExecuteScalar();
        }
        ...
        public void ReadStats(Account acc)
        {
            ...
            using (MySqlDataReader rdr = cmd.ExecuteReader())
            {
                if (rdr.HasRows)
                {
                    ...
                    acc.Onrane = rdr.GetInt32("onrane");
                    acc.Kantos = rdr.GetInt32("kantos");
                    acc.Stats = new Stats
                    {
                        Fame = rdr.GetInt32("fame"),
                        TotalFame = rdr.GetInt32("totalFame"),
                        StarsOverride = rdr.GetInt32("starOverride"),
                        StarType = rdr.GetInt32("starType")
                    };
                }
                else
                {
...
WHERE accId=@accId AND charId=@charId;";
            ...
            cmd.Parameters.AddWithValue("@markId", chr.MarkId);
...
VALUES(@accId, @petId, @objType, @skinName, @skin, @rarity, @maxLevel, @abilities, @levels, @xp);";
        ...
        public bool SaveChars(string oldAccId, Chars oldChars, Chars chrs, XmlData data)
        {
            try
            {
                ...
                var cmd = CreateQuery();
                cmd.CommandText =
                    "UPDATE accounts SET prodAcc=1, name=@name, namechosen=@nameChoosen, vaultCount=@vaults, maxCharSlot=@maxChars, ownedSkins=@skins, gifts=@gifts, enchantmentLevel=@enchantmentLevel, marksEnabled=@marksEnabled, cookingLevel=@cookingLevel WHERE id=@oldAccId;";
                ...
                cmd.Parameters.AddWithValue("@marksEnabled", chrs.Account.MarksEnabled);
                cmd.Parameters.AddWithValue("@cookingLevel", chrs.Account.CookingLevel);
                cmd.ExecuteNonQuery();
...
bestFame = GREATEST(bestFame, @bestFame);";
                ...
                cmd = CreateQuery();
                cmd.CommandText =
                    "INSERT INTO stats (accId, fame, totalFame, credits, totalCredits, fortuneTokens, totalFortuneTokens, onrane, totalOnrane, kantos, totalKantos) VALUES(@accId, @fame, @fame, @gold, @gold, @tokens, @tokens, @onrane, @onrane, @kantos, @kantos)";
                cmd.Parameters.AddWithValue("@accId", oldAccId);
                ...
                cmd.Parameters.AddWithValue("@onrane", chrs.Account.Onrane);
                cmd.Parameters.AddWithValue("@kantos", chrs.Account.Kantos);
                cmd.ExecuteNonQuery();
