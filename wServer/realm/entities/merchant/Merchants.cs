...
namespace wServer.realm.entities.merchant
{
    public class Merchants : SellableObject
    {
        ...
        private const int BUY_NO_ONRANE = 12;
        private const int BUY_NO_KANTOS = 15;
        ...
        protected override bool TryDeduct(Player player)
        {
            ...
            if (Currency == CurrencyType.Kantos)
                if (acc.Kantos < Price)
                    return false;
                        ...
                        for (var i = 0; i < player.Inventory.Length; i++)
                        {
                            try
                            {
                                ...
                                // Exploit fix - No more mnovas as weapons!
                                {
                                    ...
                                    if (Currency == CurrencyType.Onrane)
                                        player.Onrane =
                                            player.Client.Account.Onrane =
                                                db.UpdateOnraneCurrency(player.Client.Account, -Price);
                                    if (Currency == CurrencyType.Kantos)
                                        player.Kantos =
                                            player.Client.Account.Kantos =
                                                db.UpdateKantosCurrency(player.Client.Account, -Price);
                    ...
                    else
                    {
                        ...
                        switch (Currency)
                        {
                            ...
                            case CurrencyType.Fame:
                                ...
                                {
                                    Result = BUY_NO_FAME,
                                    Message = "You do not have enough fame."
                                });
                                break;
                            ...
                            case CurrencyType.FortuneTokens:
                                player.Client.SendPacket(new BuyResultPacket
                                {
                                    ...
                                    Message = "You do not have enough Fortune Tokens.",
                                });
                                break;
                            //below are the new currencies
                            case CurrencyType.Onrane:
                                player.Client.SendPacket(new BuyResultPacket
                                {
                                    Result = BUY_NO_ONRANE,
                                    Message = "You do not have enough Onrane."
                                });
                                break;
                            ...
                            case CurrencyType.Kantos:
                                player.Client.SendPacket(new BuyResultPacket
                                {
                                    Result = BUY_NO_KANTOS,
                                    Message = "You do not have enough Kantos."
                                });
                                break;
                        }
                    }
