...
namespace server
{
    public abstract class RequestHandler
    {
        ...
        public bool CheckAccount(Account acc, Database db, bool checkAccInUse = true)
        {
            ...
            if (acc.Banned)
            {
                WriteErrorLine("Account under maintenance");
                return false;
            }
            if (checkAccInUse)
            {
                ...
                {
                    if (timeout != null)
                        WriteErrorLine($"Account in use. ({timeout} seconds until timeout.)");
                    else
                        WriteErrorLine("<Error>Account in use.</Error>");
                    return false;
                }
            }
            return true;
