...
namespace wServer.networking.handlers
{
    internal class PetYardCommandHandler : PacketHandlerBase<PetYardCommandPacket>
    {
        ...
        private void UpgradePetYard(Client client, PetYardCommandPacket packet)
        {
            ...
            using (Database db = new Database())
            {
                if (packet.Currency == CurrencyType.Fame)
                {
                    ...
                    {
                        case 1:
                            if (!TryDeduct(packet.Currency, client.Player, 500))
                                return;
                            break;

                        case 2:
                            if (!TryDeduct(packet.Currency, client.Player, 2000))
                                return;
                            break;

                        case 3:
                            if (!TryDeduct(packet.Currency, client.Player, 25000))
                                return;
                            break;

                        case 4:
                            if (!TryDeduct(packet.Currency, client.Player, 50000))
                                return;
                            break;
                    }
                }
                ...
                if (packet.Currency == CurrencyType.Gold)
                {
                    ...
                    {
                        case 1:
                            if (!TryDeduct(packet.Currency, client.Player, 150))
                                return;
                            break;

                        case 2:
                            if (!TryDeduct(packet.Currency, client.Player, 400))
                                return;
                            break;

                        case 3:
                            if (!TryDeduct(packet.Currency, client.Player, 1200))
                                return;
                            break;
        ...
        private void FeedPet(Client client, PetYardCommandPacket packet)
        {
            try
            {
                ...
                if (packet.Currency == CurrencyType.Fame)
                {
                    ...
                    {
                        case Rarity.Common:
                            if (!TryDeduct(packet.Currency, client.Player, 10))
                                return;
                            break;

                        case Rarity.Uncommon:
                            if (!TryDeduct(packet.Currency, client.Player, 30))
                                return;
                            break;

                        case Rarity.Rare:
                            if (!TryDeduct(packet.Currency, client.Player, 100))
                                return;
                            break;

                        case Rarity.Legendary:
                            if (!TryDeduct(packet.Currency, client.Player, 350))
                                return;
                            break;

                        case Rarity.Divine:
                            if (!TryDeduct(packet.Currency, client.Player, 1000))
                                return;
                            break;
                ...
                if (packet.Currency == CurrencyType.Gold)
                {
                    ...
                    {
                        case Rarity.Common:
                            if (!TryDeduct(packet.Currency, client.Player, 5))
                                return;
                            break;

                        case Rarity.Uncommon:
                            if (!TryDeduct(packet.Currency, client.Player, 12))
                                return;
                            break;
                        ...
                        case Rarity.Legendary:
                            if (!TryDeduct(packet.Currency, client.Player, 60))
                                return;
                            break;
        ...
        private void FusePet(Client client, PetYardCommandPacket packet)
        {
            ...
            if (pet1.PetRarity != pet2.PetRarity)
                return;
            ...
            if (packet.Currency == CurrencyType.Fame)
            {
                ...
                {
                    case Rarity.Common:
                        if (!TryDeduct(packet.Currency, client.Player, 300))
                            return;
                        break;
                    ...
                    case Rarity.Rare:
                        if (!TryDeduct(packet.Currency, client.Player, 4000))
                            return;
                        break;

                    case Rarity.Legendary:
                        if (!TryDeduct(packet.Currency, client.Player, 15000))
                            return;
                        break;
            ...
            if (packet.Currency == CurrencyType.Gold)
            {
                switch (pet1.PetRarity)
                {
                    ...
                    case Rarity.Uncommon:
                        if (!TryDeduct(packet.Currency, client.Player, 240))
                            return;
                        break;

                    case Rarity.Rare:
                        if (!TryDeduct(packet.Currency, client.Player, 600))
                            return;
                        break;

                    case Rarity.Legendary:
                        if (!TryDeduct(packet.Currency, client.Player, 1800))
                            return;
                        break;
